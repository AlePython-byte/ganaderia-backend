...
public class CowController {
    ...
    @Operation(
            summary = "Crear vaca",
            description = "Registra una nueva vaca en el sistema. El token publico y el codigo interno son generados automaticamente por el backend; si el cliente los envia, se ignoran por compatibilidad."
    )
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "403", description = "Acceso denegado",
                    content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class))),
            @ApiResponse(responseCode = "409", description = "Conflicto al generar identificadores unicos",
                    content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class)))
    })
    ...
    @Operation(
            ...
            description = "Actualiza los datos editables de una vaca existente identificada por id. Si no se informa token, el backend conserva el valor actual; si se informa, debe seguir siendo unico. El codigo interno es generado por backend y no puede modificarse."
    )
    @ApiResponses({
